...
const Join = () => {
  ...
  // 각각의 입력창마다 유효성 검증 상태를 관리 해야 하기 때문에 객체로 선언
  // 상태를 유지하려는 이유 -> 스타일, 마지막애 회원가입 버튼 누를 때 까지 검증 상태를
  // 유지해야 하기 때문
  ...
  const saveInputState = ({ key, inputValue, msg, flag }) => {
    ...
    // 메세지 세팅
    setMessage((oldMsg) => {
      return { ...oldMsg, [key]: msg }; // key 변수의 값을 프로퍼티 키로 활용중
    });
  ...
  const nameHandler = (e) => {
    ...
    if (!inputValue) {
      ...
      msg = '2자에서 5글자 사이의 한글을 작성해주세요';
    } else {
      msg = '사용 가능한 이름 입니다.';
      flag = true;
    }

    // saveInputState 에게 이 핸들러에서 처리한 여러가지 값을 객체로 한번에 넘기기
    saveInputState({
      key: 'userName',
      inputValue,
      msg,
  ...
  const emailHandler = (e) => {
    ...
    // 중복확인 후에만 상태변경 하는 것이 아님
    // 입력창이 비거나, 정규 표현식 위반인 경우에도 상태는 변경 되어야 함!
    saveInputState({
      key: 'email',
      inputValue,
      msg,
